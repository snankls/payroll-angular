...
//   };
  calendarOptions: CalendarOptions = {
    ...
    eventClick: this.handleEventClick.bind(this),
    eventsSet: this.handleEvents.bind(this),
    selectAllow: () => false, // Disable all until employee selected
    dayCellDidMount: this.handleDayCellMount.bind(this) // Add this line
  };
  ...
  handleDayCellMount(arg: any) {
    if (!this.employeeJoiningDate) return;
  
    const cellDate = new Date(arg.date);
    cellDate.setHours(0, 0, 0, 0);
  
    const joinDate = new Date(this.employeeJoiningDate);
    joinDate.setHours(0, 0, 0, 0);
  
    if (cellDate < joinDate) {
      // Get the parent TD element
      const tdElement = arg.el.closest('.fc-day');
      if (tdElement) {
        tdElement.classList.add('fc-disabled-day');
      }
    }
  }
